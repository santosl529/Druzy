...
- **Quick-log from the grid (built):** each card is logging-first. The primary card action is "Log"; viewing the full tracker page is a secondary affordance (title / small "open" icon) so the two don't compete for the same click.
